...
VAULT_VALUES=01-vault-values-enterprise.yaml
CHECK_TIMEOUT=120
...
echo 🔧 追加：Vault Enterpriseライセンスの読み込み
VAULT_LICENSE_PATH="./vault.hclic"

if [[ -f "$VAULT_LICENSE_PATH" ]]; then
  export VAULT_LICENSE=$(cat "$VAULT_LICENSE_PATH")
  kubectl delete secret vault-enterprise-license　|| error_report "Secret削除に失敗しました。"
  kubectl create secret generic vault-enterprise-license --from-file=./vault.hclic  || error_report "Secret登録に失敗しました。"
else
  error_report "Vault Enterpriseライセンスファイルが見つかりません: $VAULT_LICENSE_PATH"
fi
...
kubectl apply -f 06-vault-init-job-enterprise.yaml || error_report "06-vault-init-job-enterprise.yaml の適用に失敗しました。"
...
kubectl apply -f 04-demo-app-deployment-enterpriseopy.yaml || error_report "04-demo-app-deployment-enterpriseopy.yaml の適用に失敗しました。"
